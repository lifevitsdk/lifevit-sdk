...
public class LifevitSDKBleDeviceTensiometer extends LifevitSDKBleDevice {
    ...
    private final static String CLASS_TAG = LifevitSDKBleDeviceTensiometer.class.getSimpleName();
    ...
    protected LifevitSDKBleDeviceTensiometer(BluetoothDevice dev, LifevitSDKManager ch) {
    ...
    protected static boolean isTensiometerDevice(String name) {
        ...
        if (UUID.fromString(UUID_BLOOD_PRESSURE_READ).equals(characteristic.getUuid())) {
            ...
            if (bytes != null && bytes.length >= 5) {
                if (bytes.length == 5 && bytes[2] == 0x06) {
                    ...
                    if (mLifevitSDKManager.getConnectionListener() != null) {
                        mLifevitSDKManager.getConnectionListener().heartDeviceOnProgressMeasurement(pulse);
                    }
